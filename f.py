...
2. **Optimization Guidelines**:
    ...
    - Each sectioin in skills should ave mininmum 3 skills
...
"""
    ...
    "Skills": PromptTemplate(
        input_variables=["generated_output", "resume_content", "job_description"],
        template="""You are a strict validator and editor. Validate and correct the provided JSON based on the rules below without altering its structure, format, or indentation. Only update values as required. If a field is invalid, inconsistent, or missing, correct it. and Remove placeholders (e.g., "xxx", "yyy") or null/empty fields,Validate the skills in the generated JSON against the skills listed in the resume content and job description, if its lacks match with job description,add skills also make sure no of skills in each section should be ,maximum 4 and maximum total no of skills should be 20,if not add if excces remove least relevant skills which match with job description
        response: {generated_output}
        Resume: {resume_content}
        Job Description: {job_description}
...
"""
    ...
    "Experience": PromptTemplate(
        input_variables=["generated_output", "resume_content", "job_description"],
        template="""You are a strict validator and editor. Validate and correct the provided JSON based on the rules below without altering its structure, format, or indentation. Only update values as required. If a field is invalid, inconsistent, or missing, correct it. If absent in the resume, remove it from the JSON entirely and Remove placeholders (e.g., "xxx", "yyy") or null/empty fields.Review and refine the following job descriptions to ensure they align closely with the job role while maintaining accuracy and consistency. Each description should include four concise bullet points structured as follows: (1) Tackled [problem/situation], (2) Took charge of [responsibilities/role], (3) Utilized [tools/technologies] to [actions], and (4) Achieved [measurable outcomes]. Ensure each point starts with a strong action verb and highlights relevant skills and accomplishments,escriptions to ensure they are clear, impactful, and align closely with the job posting requirements without overstating qualification,no of characters in eac should be 100-120 
        Response: {generated_output}
        Resume: {resume_content}
        Job Description: {job_description}
...
6. *Final Output*:
    ...
    "Projects": PromptTemplate(
        input_variables=["generated_output", "resume_content", "job_description"],
        template="""You are a strict validator and editor. Validate and correct the provided JSON based on the rules below without altering its structure, format, or indentation. Only update values as required. If a field is invalid, inconsistent, or missing, correct it. If absent in the resume, remove it from the JSON entirely.Review and refine the following job descriptions to ensure they align closely with the job role while maintaining accuracy and consistency. Each description should include four concise bullet points structured as follows: (1) Tackled [problem/situation], (2) Took charge of [responsibilities/role], (3) Utilized [tools/technologies] to [actions], and (4) Achieved [measurable outcomes]. Ensure each point starts with a strong action verb and highlights relevant skills and accomplishments,escriptions to ensure they are clear, impactful, and align closely with the job posting requirements without overstating qualification
        response: {generated_output}
        Resume: {resume_content}
        Job Description: {job_description}
...
def combined_validation(final_output, sections):
    """
    Validates and combines the final output into a LaTeX formatted resume.

    Args:
        final_output (dict): The dictionary containing validated resume sections.
        sections (dict): The dictionary with section templates.

    Returns:
        str: LaTeX formatted resume code.
    """
    
    # Define the validation prompt template
    combined_validation_prompt = PromptTemplate(
        input_variables=["final_output", "sections"],
        template=""" 
        Response: {final_output}
        LaTeX Sections: {sections}

        ### Instructions:
        You are a strict validator and editor responsible for ensuring the accuracy, consistency, and relevance of the provided JSON data. Follow these rules for each section and provide a corrected JSON with inline comments explaining the changes.
        ...
        **1. Personal Details**:
        - Ensure proper capitalization for the full name.
        - Validate email format (e.g., contains "@" and a valid domain).
        - Ensure phone numbers include the country code and are formatted properly (e.g., "+91-1234567890").
        - Verify location matches resume content (e.g., "City, Country").
        - Correct LinkedIn URLs to valid links starting with "https://www.linkedin.com/".
        - Remove missing or irrelevant fields.

        **2. Skills**:
        - Cross-check the skills with the resume for consistency.
        - Prioritize skills relevant to the JD.
        - Limit the total number of skills to 20, ensuring concise, ATS-friendly entries.
        - Organize into categories such as Languages, Frameworks, Tools, and Technologies.

        **3. Experience**:
        - Ensure consistency in job titles, company names, locations, and dates with the resume.
        - Align responsibilities and achievements with the JD by emphasizing relevant skills, tools, and technologies.
        - Use the CTAR (Context, Task, Action, Result) structure for concise descriptions.
        - Ensure each bullet is action-oriented and quantifiable where possible.

        **4. Projects**:
        - Validate project titles, tools, technologies, and descriptions against the resume.
        - Select up to 3 most relevant projects based on their alignment with the JD.
        - Follow the STAR (Situation, Task, Action, Result) method for project descriptions.
        - Limit each project to 4 concise bullet points.

        **5. Education**:
        - Validate institution names, degrees, fields of study, dates, GPA, and location with the resume.
        - Highlight relevant coursework, certifications, or skills related to the JD.
        - Remove placeholders or fields not present in the resume.
    ...
    model_type = "gemini-1.5-flash"  # Placeholder: Replace this with the actual model type if needed

    # Instantiate the LLM model with the provided API key and model type
    model_instance = genai.GenerativeModel(model_type)

    # Prepare the prompt
    prompt = combined_validation_prompt.format(final_output=final_output, sections=sections)
